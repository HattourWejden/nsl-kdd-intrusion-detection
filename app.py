...
if selected_sample != "None":
    ...
    form_data = sample_inputs["Normal"]  # default values
...
if predict_pressed:
    ...
    if pred < 0.5:
        ...
        st.markdown(
            f"<div style='background-color:#0e7b29; padding:18px; border-radius:12px; color:white; font-size:20px; font-weight:bold; text-align:center;'>"
            f"✔ Prediction: {label} (Confidence: {confidence:.1f}%)</div>",
            unsafe_allow_html=True
        )
    else:
        label = "Attack"
        confidence = pred * 100
        st.markdown(
            f"<div style='background-color:#b00020; padding:18px; border-radius:12px; color:white; font-size:20px; font-weight:bold; text-align:center;'>"
            f"⚠️ Prediction: {label} (Confidence: {confidence:.1f}%)</div>",
            unsafe_allow_html=True
        )
